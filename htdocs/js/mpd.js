...
var app = $.sammy(function () {
  ...
  this.get(/\#\/browse\/(\d+)\/(.*)/, function () {
    ...
    $("#breadcrump")
      .removeClass("hide")
      .empty()
      .append('<li><a href="#/browse/0/" onclick="set_filter()">root</a></li>');
    ...
    $.each(path_array, function (index, chunk) {
      ...
      $("#breadcrump").append(
        '<li><a href="#/browse/0/' + full_path + '">' + chunk + "</a></li>",
      );
      full_path += "/";
    });
  ...
  this.get(/\#\/dirble\/(\d+)\/(\d+)/, function () {
    ...
    $("#breadcrump")
      ...
      .empty()
      .append(
        '<li><a href="#/dirble/">Categories</a></li><li>' +
          dirble_selected_cat +
          "</li>",
      );
  ...
  this.get(/\#\/radio\//, function () {
    prepare();
    current_app = "radio";
    $("#breadcrump")
      .removeClass("hide")
      .empty()
      .append("<li>Radio Browser</li>");
    $("#salamisandwich").addClass("hide");
    $("#dirble_panel").addClass("hide");
    $("#radio_panel").removeClass("hide");
    $("#radio_results > tbody").empty();

    $("#panel-heading").text("Web Radio");
    $("#radio").addClass("active");

    // Check if there's a search term, otherwise just show empty or default search
    if ($("#radio_search_input").val()) {
      radio_load_stations($("#radio_search_input").val());
    }
  });
    ...
    socket.onmessage = function got_packet(msg) {
      ...
      switch (obj.type) {
        ...
        case "search":
          $("#wait").modal("hide");
        case "browse":
          if (current_app !== "browse" && current_app !== "search") break;
            ...
            switch (obj.data[item].type) {
              ...
              case "song":
                ...
                if (typeof obj.data[item].artist === "undefined") {
                  var details =
                    '<td colspan="2">' + obj.data[item].title + "</td>";
                } else {
                  var details =
                    "<td>" +
            ...
            $(appendTo)
              ...
              .click(function (e) {
                ...
                socket.send(
                  onClickAction +
                    "," +
                    decodeURI($(this).parents("tr").attr("uri")),
                );
                $(".top-right")
                  .notify({
                    message: {
          ...
          $("#salamisandwich > tbody > tr").on({
            click: function () {
              switch ($(this).attr("class")) {
                ...
                case "song":
                  socket.send(
                    "MPD_API_ADD_TRACK," + decodeURI($(this).attr("uri")),
                  );
                  $(".top-right")
                    .notify({
                      message: {
                ...
                case "plist":
                  socket.send(
                    "MPD_API_ADD_PLAYLIST," + decodeURI($(this).attr("uri")),
                  );
                  $(".top-right")
                    .notify({
                      message: {
        ...
        case "state":
          ...
          if (JSON.stringify(obj) === JSON.stringify(last_state)) break;
          ...
          if (obj.data.crossfade) $("#btncrossfade").addClass("active");
        ...
        case "outputnames":
          ...
          break;
        case "outputs":
          if (JSON.stringify(obj) === JSON.stringify(last_outputs)) break;
        ...
        case "mpdhost":
          ...
          if (obj.data.passwort_set) $("#mpd_password_set").removeClass("hide");
          break;
        case "dirbleapitoken":
...
var updatePlayIcon = function (state) {
  $("#play-icon").removeClass("glyphicon-play").removeClass("glyphicon-pause");
  $("#track-icon")
...
function dirble_load_categories() {
  ...
  $.getJSON(
    "https://api.dirble.com/v2/categories?token=" + dirble_api_token,
    function (data) {
...
function dirble_load_stations() {
  $.getJSON(
    "https://api.dirble.com/v2/category/" +
      dirble_catid +
      "/stations?page=" +
      dirble_page +
      "&per_page=20&token=" +
    ...
    function (data) {
      ...
      $("#dirble_left > tbody > tr > td").on({
        click: function () {
          ...
          $.getJSON(
            "https://api.dirble.com/v2/station/" +
              $(this).attr("radioid") +
              "?token=" +
              dirble_api_token,
            function (data) {
          ...
          $(this)
            ...
            .click(function (e) {
              ...
              $.getJSON(
                ...
                function (data) {
                  socket.send(
                    "MPD_API_ADD_PLAY_TRACK," + data.streams[0].stream,
                  );
                  $(".top-right")
                    .notify({
                      message: {
...
function add_filter() {
  $("#filter").append(
    '&nbsp;<a onclick="set_filter(\'#\')" href="#/browse/0/">#</a>',
  );

  for (i = 65; i <= 90; i++) {
    var c = String.fromCharCode(i);
    $("#filter").append(
      "&nbsp;<a onclick=\"set_filter('" +
        c +
        '\')" href="#/browse/0/">' +
        c +
        "</a>",
...
function render_radio_station(station) {
  var imgHtml = station.favicon
    ? '<img src="' +
      station.favicon +
  ...
  tr.append("<td>" + (station.codec || "") + "</td>");
  tr.append("<td>" + (station.bitrate || "") + "</td>");
  tr.append("<td>" + (station.country || "") + "</td>");
  tr.append("<td>" + (station.language || "") + "</td>");

  var dateStr = "";
  if (station.lastchecktime_iso8601) {
    var d = new Date(station.lastchecktime_iso8601);
    dateStr = d.toLocaleDateString();
  }
  tr.append("<td>" + dateStr + "</td>");

  var actionsTd = $(
    '<td><a role="button" class="btn btn-default btn-xs pull-right copy-btn"><span class="glyphicon glyphicon-copy"></span> Copy URL</a></td>',
  );
  tr.append(actionsTd);

  $("#radio_results > tbody").append(tr);

  // Row click
  tr.on("click", function (e) {
    socket.send("MPD_API_ADD_TRACK," + station.url_resolved);
    $(".top-right")
      .notify({ message: { text: station.name + " added to queue" } })
      .show();
  });

  // Copy button
  actionsTd.find(".copy-btn").on("click", function (e) {
    e.stopPropagation();
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(station.url_resolved).then(function () {
        $(".top-right")
          .notify({
            message: { text: "URL Copied to clipboard!", type: "success" },
            fadeOut: { enabled: true, delay: 1000 },
          })
          .show();
      });
      ...
      textArea.value = station.url_resolved;
...
function radio_load_stations(query) {
  $("#radio_loading").removeClass("hide");
  $("#radio_results > tbody").empty();

  var url =
    "https://de1.api.radio-browser.info/json/stations/search?limit=100&hidebroken=true&order=clickcount&reverse=true&name=" +
    encodeURIComponent(query);

  $.getJSON(url, function (data) {
    $("#radio_loading").addClass("hide");

    $.each(data, function (i, station) {
      render_radio_station(station);
    });

    if (data.length === 0) {
      $("#radio_results > tbody").append(
    ...
    $("#radio_loading").addClass("hide");
    $("#radio_results > tbody").append(
      '<tr><td colspan="8" class="text-center text-danger">Error fetching stations.</td></tr>',
    );
  });
}
